...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef _texture_floodfill(np.ndarray[np.uint8_t, ndim=4] lbp_image, np.ndarray[mask_t, ndim=3] mask, np.ndarray[np.float32_t, ndim=1] value, mask_t fill, float max_dist, np.ndarray[mask_t, ndim=3] strct, np.ndarray[mask_t, ndim=3] out):
...
def texture_floodfill(np.ndarray[np.uint8_t, ndim=4] lbp_image, np.ndarray[mask_t, ndim=3] mask, np.ndarray[np.float32_t, ndim=1] value, mask_t fill, float max_dist, np.ndarray[mask_t, ndim=3] strct):
    cdef np.ndarray[np.uint8_t, ndim=3] out = np.zeros_like(mask)
    _texture_floodfill(lbp_image, mask, value, fill, max_dist, strct, out)
    return out
